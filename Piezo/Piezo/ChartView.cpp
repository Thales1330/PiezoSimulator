...
void ChartView::SetTreectrl()
{
    ...
    rootElementName[static_cast<unsigned int>(ElementPlotData::CurveType::CT_SIMULAR)] = wxT("SIMULAÇÃO");
...
{
    wxFileDialog saveFileDialog(this, wxT("Salvar arquivo CSV"), "", "", "CSV file (*.csv)|*.csv",
                                wxFD_SAVE | wxFD_OVERWRITE_PROMPT);
    if(saveFileDialog.ShowModal() == wxID_CANCEL) return;
